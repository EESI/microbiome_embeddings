...
path_out <- file.path(dir_work,'seqs_ag_pca_lasso.rds')
...
train_mat <- train %>% select(-body_site,-SampleID) %>% as.matrix()
test_mat <- test %>% select(-body_site,-SampleID) %>% as.matrix()

train_mat <- prcomp(train_mat,center=TRUE,scale.=TRUE)$x[,1:256]
test_mat <- prcomp(test_mat,center=TRUE,scale.=TRUE)$x[,1:256]
