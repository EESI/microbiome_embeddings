...
train = gzip.open('/mnt/HA/groups/rosenGrp/embed_ag_samples/out/train_test_ids.csv.gz').readlines()
train = [i.decode().strip().split(',') for i in train]
train = [i for i,j in train if j == 'train']
...
for i,f in enumerate(files):
    ...
    if (sample in samples) and (sample in train):
        site = samples[sample]
...
for site in site_embedding:
    s = site.split(':')[1]
    print('Saving %s.' % (s))
    np.savetxt('/mnt/HA/groups/rosenGrp/embed_ag_samples/out/site_embedding_split_trainset/seqs_10_256_5_50_10_1e-06_100_embedding_np_%s_%s.csv' % (s,row),
